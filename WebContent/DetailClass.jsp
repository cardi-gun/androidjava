...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    ClassVO vo = list.get(0); // cCode에 맞는 데이터 vo에 담기
    String[] arr; //자격증 slicing 을 위한 배열
    int cNumOfRegist = dao.getCnumOfRegist(cCode2); //현 수강인원 수 가져오기
    if(vo.gettCrt().contains(",")){ //(DB에서 가져오면 String 이라)자격증이 2개 이상이면 , 를기준으로 나누기
    	arr = vo.gettCrt().split(",");
    }else{
    	arr = new String[]{vo.gettCrt()}; //1개면 데이터 그냥 저장
    }
    %>
<!DOCTYPE html>
<html>
						...
						<div
							...
							<div class="swiper-wrapper"
								...
								<div class="swiper-slide swiper-slide-active"
									style="color: rgb(255, 255, 255); background-image: url('images/<%=vo.getcImg()%>');background-size: cover; background-position: center center; background-repeat: no-repeat; width: 834px; height: 417px;">
					...
					<div
						...
						<div style="display: inline-block; width: 30%;">
							<span
								...
								style="display: inline-block; width: 100%; font-size: 15px; font-weight: 600; color: #2B2B2B"><%=vo.getcDate() +"  "+ vo.getcTime()+ " " + vo.getcDay()%></span>
						</div>
						<div style="display: inline-block; width: 30%;">
							<span
								style="display: inline-block; font-size: 10px; font-weight: 600; color: #646464; margin-bottom: 5px;">인원제한</span><span
								style="display: inline-block; width: 100%; font-size: 15px; font-weight: 600; color: #2B2B2B"><%=cNumOfRegist %>/<%=vo.getcLimit()+"명" %></span>
						</div>
					</div>
					<div
						...
						<div
							...
							<!-- 이미지 원형으로 만드는 코드 -->
							<div
								style="display: inline-block; vertical-align: top; background-image: url('images/<%=vo.getcImg()%>'); background-size: cover; background-position: center center; background-repeat: no-repeat; border-radius: 100px; width: 69px; height: 69px; border: 1px solid #F2F2F2;"></div>
							<div
...
// 주소로 좌표를 검색합니다
geocoder.addressSearch('<%=vo.getcAddress()%>', function(result, status) {
