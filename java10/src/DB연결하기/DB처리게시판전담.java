...
public class DB처리게시판전담 {
	public void create(String pw, String name, String tel) throws Exception {
		...
		//auto-increment때문에 첫번째 자리는 null로 입력
		String sql = "insert into bbs values(null,?,?,?)";
		//컴퓨터는 이 문장을 그냥 String으로 인식한다.
		PreparedStatement ps = con.prepareStatement(sql);
		//**db연동하는 인덱스는 1부터 시작한다!
		//auto-increment때문에 첫번째 자리는 id는 ps에 넣지 않기때문에 pw가 1번이 된다.
		ps.setString(1, pw);
		ps.setString(2, name);
		ps.setString(3, tel);
