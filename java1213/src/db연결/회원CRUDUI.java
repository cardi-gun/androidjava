...
public class 회원CRUDUI {
	...
	public static void main(String[] args) {
		...
		lbllog.setFont(new Font("굴림", Font.PLAIN, 18));
		lbllog.setBounds(268, 411, 200, 27);
		...
		login.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				MemberDAO dao = new MemberDAO();
				try {
					boolean login = dao.read(id, pw);
					if (login ==false) {
						lbllog.setText("로그인 실패. :(");
					}else {
						lbllog.setText("로그인 성공! :)");
		...
		login.setBounds(14, 411, 200, 27);
		f.getContentPane().add(login);
		
		JButton bdouble = new JButton("중복검사");
		...
		bdouble.setBounds(166, 520, 105, 27);
		f.getContentPane().add(bdouble);
